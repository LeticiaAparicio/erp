...
export class CrearProvComponent implements OnInit {
  ...
  ngOnInit() {
    this.proveedorForm = this.pf.group({
      //Aquí nos creamos el objeto del formulario
      nombre: null,
      cif: null,
      domicilio: null,
      cp: null,
  ...
  crearProv(){ //Hay que engancharlo con un backend
    ...
    this.proveedoresService.postProveedor(this.proveedor) //ese proveedor de this.proveedor es el que tenemos justo encima que es el que tiene los datos ya guardados
                    .subscribe((resp:any)=>{
                      this.router.navigate(['/proveedores']); //cuando haya éxito, nos lleva a la lista de proveedores
                      this.enviando = false;
                      // console.log(resp);
                    }, (error:any)=>{
